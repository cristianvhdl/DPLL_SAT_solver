...
bool solver::DPLL_recursively(CNF_function &curr_f) {
	...
	*/
	...
	curr_f.find_and_deduce_unit_clauses();		// find and deduce unit clauses
	curr_f.find_and_deduce_pure_literals();		// find and deduce pure literals
	...
	if (curr_f.inputs_is_empty()) {	// the set of clauses is not empty AND there are no varable left to be set -> need to back track
		...
		f_branch1.deduce(0, true);	// The first input variable is set to 1
		if (DPLL_recursively(f_branch1)) {	// solution found
			...
			f_branch2.deduce(0, false);	// The variable is set to 0
